...
const getAll = async (req, res, next) => {
  ...
  //#swagger.description=Get all the games
  const db = await mongodb.getDb()
  const result = await db.db("341Final").collection("game").find()
  result.toArray().then((lists) => {
    res.setHeader("Content-Type", "application/json")
    // #swagger.responses[200]={description: "Games found" }
...
const getSingle = async (req, res, next) => {
  //#swagger.tags=['Game']
  //#swagger.summary=Get a game
  //#swagger.description=Enter an id to find an existing game
  ...
  result.toArray().then((lists) => {
    res.setHeader("Content-Type", "application/json")
    // #swagger.responses[200]={description: "Game found" }
...
const create = async (req, res, next) => {
  //#swagger.tags=['Game']
  //#swagger.summary=Create a game
  //#swagger.description=Create a game
...
const modify = async (req, res, next) => {
  ...
  //#swagger.description=Enter an id to update an existing game
  ...
  res.setHeader("Content-Type", "application/json")
  // #swagger.responses[204]={description: "Game updated" }
...
const deleteOne = async (req, res, next) => {
  ...
  //#swagger.description=Enter an id to delete an existing game
  const gameid = new ObjectId(req.params.id)
  const db = await mongodb.getDb()
  ...
  if (checkID.length > 0) {
    ...
    res.setHeader("Content-Type", "application/json")
    // #swagger.responses[200]={description: "Game deleted" }
